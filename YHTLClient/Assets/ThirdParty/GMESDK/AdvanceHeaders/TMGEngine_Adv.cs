...
namespace TencentMobileGaming
{
    public class QAVAudioRecordCtrl : ITMGAudioRecordCtrl
    {
        ...
        public static QAVAudioRecordCtrl GetInstance()
        {
            return sInstance;
        }
        ...
        private static void s_OnAudioRecordMixComplete(int code, string dstfile, int durationMs)
        {
            if (GetInstance().OnAudioRecordMixComplete != null)
            {
                GetInstance().OnAudioRecordMixComplete(code, dstfile, durationMs);
            }
        }

        public override int StartRecord(ITMGRecordingType type, string dstFile, string accMixFile, string accPlayFile)
        {
            return QAVNative.QAVSDK_AVAudioCtrl_StartRecord((int)type, dstFile, accMixFile, accPlayFile, s_OnAudioRecordComplete);
        }
        ...
        public override int SetMixWieghts(float mic, float acc)
        {
            return QAVNative.QAVSDK_AVAudioCtrl_SetMixWieghts(mic, acc);
        }
        public override int AdjustAudioTimeByMs(int time)
        {
            return QAVNative.QAVSDK_AVAudioCtrl_AdjustAudioTimeByMs(time);
        }
        public override int MixRecordFile()
        {
            return QAVNative.QAVSDK_AVAudioCtrl_MixRecordFile(s_OnAudioRecordMixComplete);
        }
