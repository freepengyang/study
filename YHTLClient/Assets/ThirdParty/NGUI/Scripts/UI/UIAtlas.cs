...
public class UIAtlas : SFAtlas
{
    ...
    public override string ResPath
    {
        get { return mResPath; }
        set { mResPath = value; }
    }

    private bool mIsInitShader = false;
    public bool IsInitShader
    {
        get { return mIsInitShader; }
        set { mIsInitShader = value; }
    }

    /// <summary>
    /// 在资源加载中被删除了
    /// </summary>
    private bool mHasBeenDestroy;
    public override bool HasBeenDestroy
    {
        get { return mHasBeenDestroy; }
        set { mHasBeenDestroy = value; }
    }

    private bool mIsUsePoolItem = true;//false:在cssprite里面不对缓存池进行操作
    public bool IsUsePoolItem
    {
        get { return mIsUsePoolItem; }
        set { mIsUsePoolItem = value; }
    }

    public override void InitShaderMiss()
    {
        if (!mIsInitShader)
        {
            mIsInitShader = true;
            if (spriteMaterial != null && spriteMaterial.shader != null)
                spriteMaterial.shader = Shader.Find(spriteMaterial.shader.name);
        }
    }
    ...
    public override Material spriteMaterial
	{
		get
		{
