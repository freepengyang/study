...
public class UIInput : MonoBehaviour
{
    ...
    public System.Action  OnReachLimit;
	...
	protected virtual void Insert (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            {
                if (OnReachLimit != null&& (mSelectionStart!= mSelectionEnd|| Input.inputString.Length==1))
                    OnReachLimit();
                break;
            }
			if (onValidate != null) c = onValidate(sb.ToString(), sb.Length, c);
	...
	protected string GetLeftText ()
	{
		...
		return (string.IsNullOrEmpty(mValue) || min < 0||mValue.Length<min) ? "" : mValue.Substring(0, min);
	}
