...
public class UIScrollView : MonoBehaviour
{
    ...
    /// <summary>
    /// 和SetDragAmount方法功能相同，不过此方法不是直接设置，而是滑动
    /// </summary>
    /// <param name="x"></param>
    /// <param name="y"></param>
    public virtual void SetDragAmount(float x, float y, float strength = 13)
    {
        if (!mPressed)
        {
            if (mPanel == null)
            ...
            if (shouldMove)
            {
                ...
                if (canMoveHorizontally) offset.x += clip.x - ox;
                if (canMoveVertically) offset.y += clip.y - oy;

                if (offset.sqrMagnitude > 1f)
                {
                    //移动到目标点
                    Vector3 targetPos = mPanel.cachedTransform.localPosition + offset;
                    targetPos.x = Mathf.RoundToInt(targetPos.x);
                    targetPos.y = Mathf.RoundToInt(targetPos.y);
                    SpringPanel.Begin(mPanel.cachedGameObject, targetPos, strength);
                }
            }
        }
    }
    ...
    public void ScrollImmidate(float value)
    {
        if (!mIgnoreCallbacks)
        {
            mIgnoreCallbacks = true;
            float x = (movement == Movement.Horizontal) ? value : 0f;
            float y = (movement == Movement.Vertical) ? value : 0f;
            SetDragAmount(x, y, false);
            mIgnoreCallbacks = false;
        }
    }
