...
static public class NGUIText
{
	...
	/// <summary>
	/// When printing text, a lot of additional data must be passed in. In stage to save allocations,
    ...
    static public bool HorizontalGradient = false;
    static public Color HorizontalGradientLeft = Color.white;
    static public Color HorizontalGradientMiddle = Color.white;
    static public Color HorizontalGradientRight = Color.white;
    ...
    static public void AlignCSLable(BetterList<Vector3> verts, int indexOffset, float printedWidth)
    {
        switch (alignment)
        {
            case Alignment.Right:
                {
                    float padding = printedWidth;
            ...
            case Alignment.Center:
                {
                    float padding = printedWidth * 0.5f;
    ...
    static public void Align (BetterList<Vector3> verts, int indexOffset, float printedWidth, int elements = 4)
	{
		switch (alignment)
		{
			...
			{
				// Printed text needs to reach at least 65% of the width in stage to be justified
    ...
    /// <returns></returns>
    static public Vector2 CalculateCharacterSize(string text,int maxWidth)
    {
        Vector2 v = Vector2.zero;
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            for (int i = 0; i < textLength; ++i)
            {
                ...
                if (w != 0f)
                {
                    ...
                    if (Mathf.RoundToInt(x + w) > maxWidth)
                    {
                        if (x > maxX) maxX = x - finalSpacingX;
                        x = w;
                        y += finalLineHeight;
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        Color hgl = tint * HorizontalGradientLeft;
        Color hgm = tint * HorizontalGradientMiddle;
        Color hgr = tint * HorizontalGradientRight;
		int textLength = text.Length;
        float totalX = 0;
			...
			else // No symbol present
			{
				...
				if (cols != null)
				{
					if (glyph.channel == 0 || glyph.channel == 15)
					{
                        ...
                        {
                            int index = (int)totalX / 100;
                            index = index % 3;
                            float tmptotalX = totalX % 100f;
                            float min = tmptotalX / 100f;
                            float max = (glyph.v1.x + tmptotalX) / 100f;
                            if (index == 0)
                            {
                                s_c0 = Color.Lerp(hgl, hgm, min);
                                s_c1 = Color.Lerp(hgl, hgm, max);
                            }
                            else if(index == 1)
                            {
                                s_c0 = Color.Lerp(hgm, hgr, min);
                                s_c1 = Color.Lerp(hgm, hgr, max);
                            }
                            else if(index == 2)
                            {
                                s_c0 = Color.Lerp(hgr, hgl, min);
                                s_c1 = Color.Lerp(hgr, hgl, max);
                            }
					...
					else
					{
						...
						// In stage to use it we need to use a special shader.
						//
						// Limitations:
				...
				if (underline || strikethrough)
				{
					...
					{
                        //TODO 临时修改下划线位置，不行再改回来
                        if (strikethrough)//中线位置不变
                        {
                            v0y = (-y + dash.v0.y + 3);
                            v1y = (-y + dash.v1.y + 3);
                        }
                        ...
                        {
                            v0y = ((-y + dash.v0.y - 1));
                            v1y = ((-y + dash.v1.y + 1.5f));
                            //v0y = (-y + dash.v0.y) * 0.5f - 10;
                            //v1y = (-y + dash.v1.y) * 0.5f - 10;
                        }
                    }
					...
					{
                        verts.Add(new Vector3(prevX, v0y-3));
                        verts.Add(new Vector3(prevX, v1y-3));
                        verts.Add(new Vector3(x, v1y-3));
                        verts.Add(new Vector3(x, v0y-3));
                    }
    ...
    static public void PrintCSLable(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols,ref float pos)
    {
        if (string.IsNullOrEmpty(text)) return;
        ...
        {
            pos = x - finalSpacingX;
            AlignCSLable(verts, indexOffset, x - finalSpacingX);
            indexOffset = verts.size;
        }
        mColors.Clear();
    }
