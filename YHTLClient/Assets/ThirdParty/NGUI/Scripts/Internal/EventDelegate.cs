...
public class EventDelegate
{
	...
	[System.NonSerialized] Action mCachedCallback;
	...
	public EventDelegate (Action call) { Set(call); }
 ...
 #else
	static string GetMethodName (Action callback) { return callback.Method.Name; }
	static bool IsValid (Action callback) { return callback != null && callback.Method != null; }
 #endif
#else
 ...
 #else
					mCachedCallback = (Action)System.Delegate.CreateDelegate(typeof(Action), mTarget, mMethodName);
 #endif
	...
	/// This will only be used when the application is playing in stage to prevent unintentional state changes.
	/// </summary>
...
// #endif
	...
	static public EventDelegate Set (List<EventDelegate> list, Action callback)
	{
		if (list != null)
		{
	...
	static public EventDelegate Add (List<EventDelegate> list, Action callback) { return Add(list, callback, false); }

	/// <summary>
	/// Append a new event delegate to the list.
	/// </summary>

	static public EventDelegate Add (List<EventDelegate> list, Action callback, bool oneShot)
	{
		if (list != null)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
	...
	static public bool Remove (List<EventDelegate> list, Action callback)
	{
		if (list != null)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
