namespace TABLE
{
	...
	public static class GUIDESTEPHelper
	{
		public static void Encode(this System.IO.Stream stream,GUIDESTEP item)
		{
			stream.Encode(item.id);
			stream.Encode(item.show);
			stream.Encode(item.uiName);
			stream.Encode(item.uiAddress);
			stream.Encode(item.param);
			stream.Encode(item.closePanel);
			stream.Encode(item.type);
		}
	}
	public class GUIDESTEPARRAY  : ILFastMode
	{
		public GUIDESTEPARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 2;
			StringValueFixedLength = 5;
			Rules = new byte[]{1,2,2,2,2,2,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.GUIDESTEP randAttr = new TABLE.GUIDESTEP();
				handle = handles[i];
