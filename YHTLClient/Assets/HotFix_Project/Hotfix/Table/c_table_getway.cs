namespace TABLE
{
	...
	public static class GETWAYHelper
	{
		public static void Encode(this System.IO.Stream stream,GETWAY item)
		{
			stream.Encode(item.id);
			stream.Encode(item.name);
			stream.Encode(item.type);
			stream.Encode(item.function);
			stream.Encode(item.recommend);
			stream.Encode(item.Tips);
			stream.Encode(item.showLevel);
			stream.Encode(item.server);
			stream.Encode(item.upTime);
			stream.Encode(item.downTime);
			stream.Encode(item.order);
		}
	}
	public class GETWAYARRAY  : ILFastMode
	{
		public GETWAYARRAY()
		{
			...
			Rules = new byte[]{1,2,1,1,1,2,1,1,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.GETWAY randAttr = new TABLE.GETWAY();
				handle = handles[i];
