namespace TABLE
{
	...
	public static class HUNLIHelper
	{
		public static void Encode(this System.IO.Stream stream,HUNLI item)
		{
			stream.Encode(item.id);
			stream.Encode(item.type);
			stream.Encode(item.clienttip);
			stream.Encode(item.plus);
			stream.Encode(item.per);
			stream.Encode(item.clientattr);
		}
	}
	public class HUNLIARRAY  : ILFastMode
	{
		public HUNLIARRAY()
		{
		...
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.HUNLI randAttr = new TABLE.HUNLI();
				handle = handles[i];
