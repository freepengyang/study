namespace TABLE
{
	public partial class SHIZHENTYPE
	{
	...
	public static class SHIZHENTYPEHelper
	{
		public static void Encode(this System.IO.Stream stream,SHIZHENTYPE item)
		{
			stream.Encode(item.id);
			stream.Encode(item.type);
			stream.Encode(item.rewordNum);
			stream.Encode(item.probabily);
			stream.Encode(item.layer);
			stream.Encode(item.shizhensize);
		}
	}
	public class SHIZHENTYPEARRAY  : ILFastMode
	{
		public SHIZHENTYPEARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 3;
			StringValueFixedLength = 3;
			Rules = new byte[]{1,1,2,1,2,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.SHIZHENTYPE randAttr = new TABLE.SHIZHENTYPE();
				handle = handles[i];
