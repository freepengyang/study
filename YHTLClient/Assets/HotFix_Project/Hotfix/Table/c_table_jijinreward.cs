namespace TABLE
{
	public partial class JIJINREWARD
	{
	...
	public static class JIJINREWARDHelper
	{
		public static void Encode(this System.IO.Stream stream,JIJINREWARD item)
		{
			stream.Encode(item.id);
			stream.Encode(item.type);
			stream.Encode(item.reward);
			stream.Encode(item.num);
			stream.Encode(item.needIntegral);
			stream.Encode(item.loopSign);
			stream.Encode(item.loopIntegral);
			stream.Encode(item.tips);
		}
	}
	public class JIJINREWARDARRAY  : ILFastMode
	{
		public JIJINREWARDARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 7;
			StringValueFixedLength = 1;
			Rules = new byte[]{1,1,1,1,1,1,1,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.JIJINREWARD randAttr = new TABLE.JIJINREWARD();
				handle = handles[i];
