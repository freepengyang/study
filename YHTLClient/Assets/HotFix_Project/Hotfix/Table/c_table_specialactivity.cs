namespace TABLE
{
	public partial class SPECIALACTIVITY
	{
	...
	public static class SPECIALACTIVITYHelper
	{
		public static void Encode(this System.IO.Stream stream,SPECIALACTIVITY item)
		{
			stream.Encode(item.id);
			stream.Encode(item.eventId);
			stream.Encode(item.eventName);
			stream.Encode(item.smallName);
			stream.Encode(item.eventType);
			stream.Encode(item.showitem);
			...
			stream.Encode(item.open);
		}
	}
	public class SPECIALACTIVITYARRAY  : ILFastMode
	{
		public SPECIALACTIVITYARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 9;
			StringValueFixedLength = 4;
			Rules = new byte[]{1,1,2,2,1,2,1,1,1,2,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.SPECIALACTIVITY randAttr = new TABLE.SPECIALACTIVITY();
				handle = handles[i];
