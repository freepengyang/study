namespace TABLE
{
	...
	public static class RECHARGEREWARDHelper
	{
		public static void Encode(this System.IO.Stream stream,RECHARGEREWARD item)
		{
			stream.Encode(item.id);
			stream.Encode(item.type);
			stream.Encode(item.loop);
			stream.Encode(item.need);
			stream.Encode(item.box);
			stream.Encode(item.desc);
		}
	}
	public class RECHARGEREWARDARRAY  : ILFastMode
	{
		public RECHARGEREWARDARRAY()
		{
		...
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.RECHARGEREWARD randAttr = new TABLE.RECHARGEREWARD();
				handle = handles[i];
