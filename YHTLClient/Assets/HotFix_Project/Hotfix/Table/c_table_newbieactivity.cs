namespace TABLE
{
	public partial class NEWBIEACTIVITY
	{
	...
	public static class NEWBIEACTIVITYHelper
	{
		public static void Encode(this System.IO.Stream stream,NEWBIEACTIVITY item)
		{
			stream.Encode(item.id);
			stream.Encode(item.name);
			stream.Encode(item.group);
			stream.Encode(item.desc1);
			stream.Encode(item.unlockDay);
			...
			stream.Encode(item.score);
			stream.Encode(item.deliver);
			stream.Encode(item.uiModel);
		}
	}
	public class NEWBIEACTIVITYARRAY  : ILFastMode
	{
		public NEWBIEACTIVITYARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 10;
			StringValueFixedLength = 4;
			Rules = new byte[]{1,2,1,2,1,1,2,1,1,1,2,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.NEWBIEACTIVITY randAttr = new TABLE.NEWBIEACTIVITY();
				handle = handles[i];
