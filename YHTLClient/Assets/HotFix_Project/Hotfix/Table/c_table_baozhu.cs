namespace TABLE
{
	...
	public static class BAOZHUHelper
	{
		public static void Encode(this System.IO.Stream stream,BAOZHU item)
		{
			stream.Encode(item.id);
			stream.Encode(item.quality);
			stream.Encode(item.grade);
			stream.Encode(item.exp);
			stream.Encode(item.addAtribute);
		}
	}
	public class BAOZHUARRAY  : ILFastMode
	{
		public BAOZHUARRAY()
		{
		...
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.BAOZHU randAttr = new TABLE.BAOZHU();
				handle = handles[i];
