namespace TABLE
{
	public partial class JINGJIHUODONG
	{
	...
	public static class JINGJIHUODONGHelper
	{
		public static void Encode(this System.IO.Stream stream,JINGJIHUODONG item)
		{
			stream.Encode(item.id);
			stream.Encode(item.eventId);
			stream.Encode(item.type);
			stream.Encode(item.level);
			stream.Encode(item.name1);
			stream.Encode(item.name2);
			stream.Encode(item.uiModel);
			stream.Encode(item.tip);
			stream.Encode(item.open2);
			stream.Encode(item.close2);
			stream.Encode(item.showdate);
			stream.Encode(item.marker);
		}
	}
	public class JINGJIHUODONGARRAY  : ILFastMode
	{
		public JINGJIHUODONGARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 7;
			StringValueFixedLength = 5;
			Rules = new byte[]{1,2,1,1,2,2,1,2,1,1,1,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.JINGJIHUODONG randAttr = new TABLE.JINGJIHUODONG();
				handle = handles[i];
