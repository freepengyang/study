namespace TABLE
{
	...
	{
		public static void Encode(this System.IO.Stream stream,TASKS item)
		{
			...
			stream.Encode(item.goals);
			stream.Encode(item.rewards);
			stream.Encode(item.fromNPC);
			stream.Encode(item.toNPC);
			stream.Encode(item.tip1);
			stream.Encode(item.tip2);
			stream.Encode(item.tip3);
			stream.Encode(item.tip4);
			stream.Encode(item.yindao);
			stream.Encode(item.transfer);
			stream.Encode(item.completeWay);
			stream.Encode(item.roundLimit);
			stream.Encode(item.cost);
			stream.Encode(item.subtype);
			stream.Encode(item.quick);
			stream.Encode(item.goalTips1);
			stream.Encode(item.goalTips2);
		}
	}
	public class TASKSARRAY  : ILFastMode
	{
		public TASKSARRAY()
		{
			...
			Rules = new byte[]{1,2,1,1,1,1,1,2,1,1,2,2,2,2,1,1,2,1,2,1,2,2,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.TASKS randAttr = new TABLE.TASKS();
				handle = handles[i];
