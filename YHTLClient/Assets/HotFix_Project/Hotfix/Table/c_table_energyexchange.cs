namespace TABLE
{
	public partial class ENERGYEXCHANGE
	{
	...
	public static class ENERGYEXCHANGEHelper
	{
		public static void Encode(this System.IO.Stream stream,ENERGYEXCHANGE item)
		{
			stream.Encode(item.id);
			stream.Encode(item.times);
			stream.Encode(item.energy);
			stream.Encode(item.costType1);
			stream.Encode(item.costNum1);
			stream.Encode(item.costType2);
			stream.Encode(item.costNum2);
			stream.Encode(item.mutexCostNum);
		}
	}
	public class ENERGYEXCHANGEARRAY  : ILFastMode
	{
		public ENERGYEXCHANGEARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 8;
			StringValueFixedLength = 0;
			Rules = new byte[]{1,1,1,1,1,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.ENERGYEXCHANGE randAttr = new TABLE.ENERGYEXCHANGE();
				handle = handles[i];
