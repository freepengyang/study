namespace TABLE
{
	...
	{
		public static void Encode(this System.IO.Stream stream,RECHARGE item)
		{
			stream.Encode(item.id);
			stream.Encode(item.gold);
			stream.Encode(item.money);
			stream.Encode(item.bonusBox);
			stream.Encode(item.imgSrc);
			stream.Encode(item.id7_money20);
			stream.Encode(item.channelControl);
			stream.Encode(item.times);
			stream.Encode(item.vip);
			stream.Encode(item.order);
		}
	}
	public class RECHARGEARRAY  : ILFastMode
	{
		public RECHARGEARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 7;
			StringValueFixedLength = 3;
			Rules = new byte[]{1,1,1,1,2,1,1,2,2,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.RECHARGE randAttr = new TABLE.RECHARGE();
				handle = handles[i];
