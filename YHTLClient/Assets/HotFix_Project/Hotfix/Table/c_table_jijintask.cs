namespace TABLE
{
	public partial class JIJINTASK
	{
	...
	public static class JIJINTASKHelper
	{
		public static void Encode(this System.IO.Stream stream,JIJINTASK item)
		{
			stream.Encode(item.id);
			stream.Encode(item.name);
			stream.Encode(item.type);
			stream.Encode(item.taskType);
			stream.Encode(item.typeParameter);
			stream.Encode(item.count);
			stream.Encode(item.showType);
			stream.Encode(item.num);
			stream.Encode(item.getIntegral);
			stream.Encode(item.deliver);
			stream.Encode(item.uiModel);
			stream.Encode(item.tips);
		}
	}
	public class JIJINTASKARRAY  : ILFastMode
	{
		public JIJINTASKARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 10;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,2,1,1,2,1,1,1,1,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.JIJINTASK randAttr = new TABLE.JIJINTASK();
				handle = handles[i];
