namespace TABLE
{
	...
	{
		public static void Encode(this System.IO.Stream stream,EFFECT item)
		{
			stream.Encode(item.id);
			stream.Encode(item.name);
			stream.Encode(item.resType);
			stream.Encode(item.destroyTime);
			stream.Encode(item.effecttype);
			stream.Encode(item.offsetX);
			stream.Encode(item.offsetY);
			stream.Encode(item.offsetZ);
			stream.Encode(item.playType);
		}
	}
	public class EFFECTARRAY  : ILFastMode
	{
		public EFFECTARRAY()
		{
		...
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.EFFECT randAttr = new TABLE.EFFECT();
				handle = handles[i];
