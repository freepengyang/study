namespace TABLE
{
	...
	public static class BIQISHOPHelper
	{
		public static void Encode(this System.IO.Stream stream,BIQISHOP item)
		{
			stream.Encode(item.id);
			stream.Encode(item.type);
			stream.Encode(item.subType);
			stream.Encode(item.boxId);
			stream.Encode(item.name);
			stream.Encode(item.value);
			stream.Encode(item.value2);
	...
	public class BIQISHOPARRAY  : ILFastMode
	{
		public BIQISHOPARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 9;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,1,1,1,2,1,1,1,1,2,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.BIQISHOP randAttr = new TABLE.BIQISHOP();
				handle = handles[i];
