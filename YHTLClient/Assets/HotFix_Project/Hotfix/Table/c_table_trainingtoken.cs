namespace TABLE
{
	public partial class TRAININGTOKEN
	{
	...
	public static class TRAININGTOKENHelper
	{
		public static void Encode(this System.IO.Stream stream,TRAININGTOKEN item)
		{
			stream.Encode(item.id);
			stream.Encode(item.lvFloor);
			stream.Encode(item.lvUp);
			stream.Encode(item.task);
			stream.Encode(item.monsterLv);
			stream.Encode(item.baseExp);
			stream.Encode(item.show);
		}
	}
	public class TRAININGTOKENARRAY  : ILFastMode
	{
		public TRAININGTOKENARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 5;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,1,1,1,2,1,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.TRAININGTOKEN randAttr = new TABLE.TRAININGTOKEN();
				handle = handles[i];
