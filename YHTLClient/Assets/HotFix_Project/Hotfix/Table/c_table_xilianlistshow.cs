namespace TABLE
{
	public partial class XILIANLISTSHOW
	{
	...
	public static class XILIANLISTSHOWHelper
	{
		public static void Encode(this System.IO.Stream stream,XILIANLISTSHOW item)
		{
			stream.Encode(item.id);
			stream.Encode(item.subType);
			stream.Encode(item.levClass);
			stream.Encode(item.type);
			stream.Encode(item.keyid);
			stream.Encode(item.color);
			stream.Encode(item.parameter);
			stream.Encode(item.Max);
		}
	}
	public class XILIANLISTSHOWARRAY  : ILFastMode
	{
		public XILIANLISTSHOWARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 6;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,1,1,1,1,2,2,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.XILIANLISTSHOW randAttr = new TABLE.XILIANLISTSHOW();
				handle = handles[i];
