namespace TABLE
{
	public partial class BAOZHUJINHUA
	{
	...
	public static class BAOZHUJINHUAHelper
	{
		public static void Encode(this System.IO.Stream stream,BAOZHUJINHUA item)
		{
			stream.Encode(item.id);
			stream.Encode(item.rank);
			stream.Encode(item.addRandomAttr);
			stream.Encode(item.maxLevel);
			stream.Encode(item.skillSlotID);
			stream.Encode(item.bossLevel);
			stream.Encode(item.bossNum);
			stream.Encode(item.quality);
			stream.Encode(item.levClass);
			stream.Encode(item.gradename);
		}
	}
	public class BAOZHUJINHUAARRAY  : ILFastMode
	{
		public BAOZHUJINHUAARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 9;
			StringValueFixedLength = 1;
			Rules = new byte[]{1,1,1,1,1,1,1,1,1,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.BAOZHUJINHUA randAttr = new TABLE.BAOZHUJINHUA();
				handle = handles[i];
