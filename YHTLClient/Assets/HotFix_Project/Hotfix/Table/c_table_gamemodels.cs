namespace TABLE
{
	...
	public static class GAMEMODELSHelper
	{
		public static void Encode(this System.IO.Stream stream,GAMEMODELS item)
		{
			stream.Encode(item.id);
			stream.Encode(item.model);
			stream.Encode(item.modelName);
			stream.Encode(item.layer);
			stream.Encode(item.subLayer);
			stream.Encode(item.functionId);
		}
	}
	public class GAMEMODELSARRAY  : ILFastMode
	{
		public GAMEMODELSARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 4;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,2,2,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.GAMEMODELS randAttr = new TABLE.GAMEMODELS();
				handle = handles[i];
