namespace TABLE
{
	...
	{
		public static void Encode(this System.IO.Stream stream,FASHION item)
		{
			stream.Encode(item.id);
			stream.Encode(item.fashionid);
			stream.Encode(item.name);
			stream.Encode(item.type);
			stream.Encode(item.sex);
			stream.Encode(item.star);
			stream.Encode(item.cost);
			stream.Encode(item.phy);
			stream.Encode(item.magic);
			stream.Encode(item.tao);
			stream.Encode(item.skill);
			stream.Encode(item.combineItem);
			stream.Encode(item.combineNum);
			stream.Encode(item.clothesModel);
			stream.Encode(item.weaponryModel);
			stream.Encode(item.titleModel);
			stream.Encode(item.icon);
			stream.Encode(item.quality);
			stream.Encode(item.getWay);
		}
	}
	public class FASHIONARRAY  : ILFastMode
	{
		public FASHIONARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 12;
			StringValueFixedLength = 7;
			Rules = new byte[]{1,1,2,1,1,1,2,2,2,2,1,1,1,1,1,1,2,1,2};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.FASHION randAttr = new TABLE.FASHION();
				handle = handles[i];
