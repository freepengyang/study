namespace TABLE
{
	...
	{
		public static void Encode(this System.IO.Stream stream,ITEMLEVEL item)
		{
			stream.Encode(item.id);
			stream.Encode(item.deliverId);
			stream.Encode(item.itemLevel);
			stream.Encode(item.show);
			stream.Encode(item.num);
			stream.Encode(item.bossCount);
		}
	}
	public class ITEMLEVELARRAY  : ILFastMode
	{
		public ITEMLEVELARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 4;
			StringValueFixedLength = 2;
			Rules = new byte[]{1,1,2,2,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.ITEMLEVEL randAttr = new TABLE.ITEMLEVEL();
				handle = handles[i];
