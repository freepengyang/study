namespace TABLE
{
	public partial class ACTIVITYBUTTON
	{
		...
		public IntArray redpoints
		{
			get
			{
				IntArray array;
				array.__fastMode = this.Array;
				array.__start = redpointsPtr & 0xFFFFF;
				array.__length = (redpointsPtr >> 20) & 0xFFF;
				array.__end = array.__length + array.__start;
				return array;
			}
		}
	}
	public static class ACTIVITYBUTTONHelper
	{
		public static void Encode(this System.IO.Stream stream,ACTIVITYBUTTON item)
		{
			stream.Encode(item.id);
			stream.Encode(item.name);
			stream.Encode(item.order);
			stream.Encode(item.layer);
			stream.Encode(item.functionId);
			stream.Encode(item.resident);
			stream.Encode(item.residenteffect);
			stream.Encode(item.redpointsPtr);
		}
	}
	public class ACTIVITYBUTTONARRAY  : ILFastMode
	{
		public ACTIVITYBUTTONARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 7;
			StringValueFixedLength = 1;
			Rules = new byte[]{1,2,1,1,1,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.ACTIVITYBUTTON randAttr = new TABLE.ACTIVITYBUTTON();
				handle = handles[i];
