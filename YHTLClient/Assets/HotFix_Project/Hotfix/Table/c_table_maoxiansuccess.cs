namespace TABLE
{
	public partial class MAOXIANSUCCESS
	{
	...
	public static class MAOXIANSUCCESSHelper
	{
		public static void Encode(this System.IO.Stream stream,MAOXIANSUCCESS item)
		{
			stream.Encode(item.id);
			stream.Encode(item.levelID);
			stream.Encode(item.rate1);
			stream.Encode(item.factor1);
			stream.Encode(item.factorNPC1);
			stream.Encode(item.rate2);
			stream.Encode(item.factor2);
			stream.Encode(item.factorNPC2);
		}
	}
	public class MAOXIANSUCCESSARRAY  : ILFastMode
	{
		public MAOXIANSUCCESSARRAY()
		{
			LongValueFixedLength = 0;
			IntValueFixedLength = 7;
			StringValueFixedLength = 1;
			Rules = new byte[]{1,1,1,2,1,1,1,1};
		}
		public override void Decode(byte[] contents)
		{
			...
			{
				TABLE.MAOXIANSUCCESS randAttr = new TABLE.MAOXIANSUCCESS();
				handle = handles[i];
