...
/// <summary>
/// 此代码摘自网络 由c++翻译而来 - by 沈少军
/// </summary>
namespace Skyiv.Util
{
	public class RB_Tree<KEY, U> where KEY : class,new() where U : class, new()
	{
		public enum COLOR { RED, BLACK };
		public class RB_Node
		{
			public COLOR RB_COLOR;
			public RB_Node right;
			public RB_Node left;
		...
		public delegate int KeyComparer(KEY l, KEY r);
		public KeyComparer m_comparer;

		public PoolHandleManager m_poolHandleManager;

		RB_Node Get()
		{
			return m_poolHandleManager.GetSystemClass<RB_Node>();
		}

		void Recycle(RB_Node node)
		{
			node.Reset();
			m_poolHandleManager.Recycle(node);
		}

		public RB_Tree(PoolHandleManager poolHandleManager,KeyComparer comparer)
		{
			m_poolHandleManager = poolHandleManager;
			m_comparer = comparer;
			m_nullNode = Get();
			m_root = m_nullNode;
			...
			m_nullNode.RB_COLOR = COLOR.BLACK;
		}
		...
		public bool next()
		{
			...
			return m_current != m_nullNode;
		}

		public RB_Node value()
		{
			return m_current;
		}
		...
		RB_Node find(KEY key)
		{
			...
			{
				int compareValue = m_comparer(key, index.key);
				if (compareValue < 0)
				{
		...
		*/
		...
		public bool Add(KEY key, U data)
		{
			...
			RB_Node insert_node = Get();
			insert_node.key = key;
			insert_node.data = data;
			insert_node.RB_COLOR = COLOR.RED;
			...
			{
				if (m_comparer(key,insert_point.key) < 0)
				{
		...
		*/
		...
		{
			while (node.parent.RB_COLOR == COLOR.RED)
			{
				if (node.parent == node.parent.parent.left)   //
				{
					RB_Node uncle = node.parent.parent.right;
					if (uncle.RB_COLOR == COLOR.RED)   //插入情况1，z的叔叔y是红色的。
					{
						node.parent.RB_COLOR = COLOR.BLACK;
						uncle.RB_COLOR = COLOR.BLACK;
						node.parent.parent.RB_COLOR = COLOR.RED;
						node = node.parent.parent;
					}
					else if (uncle.RB_COLOR == COLOR.BLACK)  //插入情况2：z的叔叔y是黑色的，。
					{
					 ...
					 //15 else (same as then clause with "right" and "left" exchanged)
				{
					...
					if (uncle.RB_COLOR == COLOR.RED)
					{
						node.parent.RB_COLOR = COLOR.BLACK;
						uncle.RB_COLOR = COLOR.BLACK;
						uncle.parent.RB_COLOR = COLOR.RED;
						node = node.parent.parent;
					}
					else if (uncle.RB_COLOR == COLOR.BLACK)
					{
		...
		public bool Remove(KEY key)
		{
			...
			{
				RB_Node successor = Successor(delete_point);
			...
			if (delete_point.RB_COLOR == COLOR.BLACK && !(delete_point_child == m_nullNode && delete_point_child.parent == m_nullNode))
			{
				DeleteFixUp(delete_point_child);
			}
			Recycle(delete_point);
			return true;
		}
		...
		*/
		...
		{
			while (node != m_root && node.RB_COLOR == COLOR.BLACK)
			{
				if (node == node.parent.left)
				{
					RB_Node brother = node.parent.right;
					if (brother.RB_COLOR == COLOR.RED)   //情况1：x的兄弟w是红色的。
					{
						brother.RB_COLOR = COLOR.BLACK;
						node.parent.RB_COLOR = COLOR.RED;
						RotateLeft(node.parent);
					}
					else     //情况2：x的兄弟w是黑色的，
					{
						if (brother.left.RB_COLOR == COLOR.BLACK && brother.right.RB_COLOR == COLOR.BLACK)
						//且w的俩个孩子都是黑色的。
						{
							brother.RB_COLOR = COLOR.RED;
							node = node.parent;
						}
						else if (brother.right.RB_COLOR == COLOR.BLACK)
						//情况3：x的兄弟w是黑色的，w的右孩子是黑色（w的左孩子是红色）。
						{
							brother.RB_COLOR = COLOR.RED;
							brother.left.RB_COLOR = COLOR.BLACK;
							RotateRight(brother);
						}
						else if (brother.right.RB_COLOR == COLOR.RED)
						//情况4：x的兄弟w是黑色的，且w的右孩子时红色的。
						{
							brother.RB_COLOR = node.parent.RB_COLOR;
							node.parent.RB_COLOR = COLOR.BLACK;
							brother.right.RB_COLOR = COLOR.BLACK;
							RotateLeft(node.parent);
							node = m_root;
						}
					}
					  ...
					  //同样，原理一致，只是遇到左旋改为右旋，遇到右旋改为左旋，即可。其它代码不变。
				{
					...
					else
					{
						...
						else if (brother.left.RB_COLOR == COLOR.BLACK)
						{
							brother.RB_COLOR = COLOR.RED;
							brother.right.RB_COLOR = COLOR.BLACK;
							RotateLeft(brother);
						}
						else if (brother.left.RB_COLOR == COLOR.RED)
						{
							brother.RB_COLOR = node.parent.RB_COLOR;
							node.parent.RB_COLOR = COLOR.BLACK;
							brother.left.RB_COLOR = COLOR.BLACK;
			...
			node.RB_COLOR = COLOR.BLACK;    //并改为黑色。
		}

		//
		RB_Node Precursor(RB_Node node)
		{
