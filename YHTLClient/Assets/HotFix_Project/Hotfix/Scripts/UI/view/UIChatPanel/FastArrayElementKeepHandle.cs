...
public class FastArrayElementKeepHandle<T> where T : class
{
    ...
    public delegate void MakeSortHandle(ref long sortedValue, T value);
    ...
    public void SwapErase(int i)
    {
        if (i >= 0 && i < Count)
        {
            ...
            {
                //mBuffer[i] = default(T);
                System.Array.Clear(mBuffer, i, 1);
                --Count;
            }
            else
            {
                ...
                System.Array.Clear(mBuffer, Count, 1);
    ...
    public void SwapRemove(T element)
    {
        for (int i = 0, max = Count; i < max; ++i)
        {
            if (mBuffer[i] == element)
