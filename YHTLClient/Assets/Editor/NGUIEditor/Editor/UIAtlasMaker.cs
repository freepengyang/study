...
public class UIAtlasMaker : EditorWindow
{
    ...
    public delegate void OnPackFaillCallBack(UIAtlas atlas);
    ...
    static bool PackTextures(Texture2D tex, List<SpriteEntry> sprites)
    {
		...
		int maxSize = 4096;
#else
        int maxSize = /*SystemInfo.maxTextureSize*/4096;
#endif
    ...
    static public List<SpriteEntry> CreateSprites(List<Texture> textures, bool isRefreshTexImport = true)
    {
        List<SpriteEntry> list = new List<SpriteEntry>();

        foreach (Texture tex in textures)
        {
            Texture2D oldTex = NGUIEditorTools.ImportTexture(tex, true, false, isRefreshTexImport);
    ...
    static public bool UpdateTexture(UIAtlas atlas, List<SpriteEntry> sprites,OnPackFaillCallBack callback = null)
    {
        ...
        if (System.IO.File.Exists(newPath))
        {
            ...
            {
                FNDebug.LogError(newPath + " is not editable. Did you forget to do a check out?");
                return false;
            }
#endif
        ...
        if (PackTextures(tex, sprites))
        {
            ...
            if (newTexture)
            {
                if (tex == null) FNDebug.LogError("Failed to load the created atlas saved as " + newPath);
    ...
    /// Show a progress bar.
    /// </summary>
    public static string ProgressReplaceStr;
    static public void ShowProgress(float val)
    {
        if (string.IsNullOrEmpty(ProgressReplaceStr))
        {
            EditorUtility.DisplayProgressBar("Updating", "Updating the atlas, please wait...", val);
        }
        else
        {
            EditorUtility.DisplayProgressBar("Updating", ProgressReplaceStr, val);
        }
    ...
    static public void AddOrUpdate(UIAtlas atlas, Texture2D tex)
    {
        if (atlas != null && tex != null)
        {
            ...
            sprites = CreateSprites(textures);
    ...
    public static void UpdateAtlas(List<Texture> textures, bool keepSprites, bool isSelectAtlas = true, bool isRefreshTexImport = true, OnPackFaillCallBack callback = null)
    {
        // Create a list of sprites using the collected textures
        List<SpriteEntry> sprites;
        sprites = CreateSprites(textures, isRefreshTexImport);
        if (sprites.Count > 0)
        {
            ...
            // Update the atlas
            UpdateAtlas(NGUISettings.atlas, sprites, isSelectAtlas, callback);
        }
        else if (!keepSprites)
        {
            UpdateAtlas(NGUISettings.atlas, sprites, isSelectAtlas, callback);
    ...
    static public void UpdateAtlas(UIAtlas atlas, List<SpriteEntry> sprites, bool isSelectAtlas = true, OnPackFaillCallBack callback = null)
    {
        if (sprites.Count > 0)
        {
            // Combine all sprites into a single texture and save it
            if (UpdateTexture(atlas, sprites,callback))
            {
                // Replace the sprites within the atlas
        ...
        atlas.MarkAsChanged();
        if(isSelectAtlas)Selection.activeGameObject = (NGUISettings.atlas != null) ? NGUISettings.atlas.gameObject : null;
        EditorUtility.ClearProgressBar();
    }
    ...
    void OnGUI()
    {
        ...
        if (spriteList.Count > 0)
        {
            NGUIEditorTools.DrawHeader("Sprites", true);
            {
                ...
                {
                    if (NGUISettings.atlas != null && NGUISettings.atlas.spriteMaterial != null)
                    {
                        EditorUtil.SetTexReadable((Texture2D)NGUISettings.atlas.spriteMaterial.mainTexture, true);
                    }
                }
                ...
                if (delete || update || replace)
                {
                    if (NGUISettings.atlas != null && NGUISettings.atlas.spriteMaterial != null)
                    {
                        EditorUtil.SetTexReadable((Texture2D)NGUISettings.atlas.spriteMaterial.mainTexture, false);
                    }
                }
            }
        }
    ...
    public static void RemoveExtractSprite(List<Texture> texList)
    {
        SetAtlasReadable(true);
        List<SpriteEntry> sprites = new List<SpriteEntry>();
        ExtractSprites(NGUISettings.atlas, sprites);
        ...
        {
            SpriteEntry ent = sprites[--i];
            Texture tex = texList.Find((f) =>
                {
                    return f.name == ent.name;
                });
            if (tex==null)
                sprites.RemoveAt(i);
        }
        UpdateAtlas(NGUISettings.atlas, sprites);
        NGUIEditorTools.RepaintSprites();
        SetAtlasReadable(false);
    }

    public static void SetAtlasReadable(bool isReadable)
    {
        if (NGUISettings.atlas != null && NGUISettings.atlas.spriteMaterial != null)
        {
            EditorUtil.SetTexReadable((Texture2D)NGUISettings.atlas.spriteMaterial.mainTexture, isReadable);
        }
    }
}
