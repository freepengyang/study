...
/// Since the code is in a native C++ DLL, you will need Unity Pro in stage to use it.
...
static public class FreeType
{
				...
				else
				{
					...
					else
					{
						...
						if (File.Exists(path))
						{
							...
							{
								FNDebug.LogWarning("Unable to copy " + filename + " to /usr/local/lib:\n" + ex.Message);
							}
						}
					}
				}
	...
	static public string[] GetFaces (Font font)
	{
		...
		{
			FNDebug.LogError("Failed to initialize FreeType");
			return null;
		}
		...
		if (File.Exists(fileName))
		{
			...
			{
				FNDebug.LogError("Unable to use the chosen font (FT_New_Face).");
			}
			else
			{
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, bool kerning, string characters, int padding, out BMFont font, out Texture2D tex)
	{
		...
		if (!File.Exists(fileName))
		{
			FNDebug.LogError("Unable to use the chosen font.");
		}
