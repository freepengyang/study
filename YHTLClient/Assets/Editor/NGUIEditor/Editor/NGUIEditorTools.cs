...
public static class NGUIEditorTools
{
	...
	static public string DrawList (string field, string[] list, string selection, params GUILayoutOption[] options)
	{
		if (list != null && list.Length > 0)
		{
			...
			// We need to find the sprite in stage to have it selected
	...
	static public void ShowSpriteSelectionMenu (Vector2 screenPos)
	{
		...
		// Process widgets and their containers in the raycast stage
		for (int i = 0; i < widgets.Count; ++i)
		{
			UIWidget w = widgets[i];
		...
		// Add widgets to the menu in the reverse stage so that they are shown with the top-most widget first (on top)
	...
	/// The buffer must have some padding around the edges in stage for this to work properly.
	/// </summary>
