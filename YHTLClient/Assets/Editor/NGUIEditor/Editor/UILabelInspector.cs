...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
            ...
            #region FormatStr(Lxh)
            SerializedProperty formatP = serializedObject.FindProperty("FormatStr");
            if (formatP.hasMultipleDifferentValues)
            {
                NGUIEditorTools.DrawProperty("", formatP, GUILayout.Height(128f));
            }
            else
            {
                GUIStyle style = new GUIStyle(EditorStyles.textField);
                ...
                float height = style.CalcHeight(new GUIContent(formatP.stringValue), Screen.width - 100f);
                bool offset = true;

                if (height > 90f)
                {
                    offset = false;
                    height = style.CalcHeight(new GUIContent(formatP.stringValue), Screen.width - 20f);
                }
                else
                {
                    GUILayout.BeginHorizontal();
                ...
                GUI.changed = false;
                string text = EditorGUI.TextArea(rect, formatP.stringValue, style);
                if (GUI.changed) formatP.stringValue = text;
            ...
            EditorGUI.BeginDisabledGroup(mLabel.bitmapFont != null && mLabel.bitmapFont.packedFontShader);
            GUILayout.BeginHorizontal();
            SerializedProperty horizontalgr = NGUIEditorTools.DrawProperty("Horizontal Gradient", serializedObject, "mApplyHorizontalGradient",
            GUILayout.Width(95f));

            EditorGUI.BeginDisabledGroup(!horizontalgr.hasMultipleDifferentValues && !horizontalgr.boolValue);
            {
                NGUIEditorTools.SetLabelWidth(30f);
                NGUIEditorTools.DrawProperty("Left", serializedObject, "mHorizontalGradientLeft", GUILayout.MinWidth(40f));
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                NGUIEditorTools.SetLabelWidth(50f);
                GUILayout.Space(79f);
                NGUIEditorTools.DrawProperty("Middle", serializedObject, "mHorizontalGradientMiddle", GUILayout.MinWidth(40f));
                NGUIEditorTools.SetLabelWidth(80f);
                NGUIEditorTools.DrawProperty("Right", serializedObject, "mHorizontalGradientRight", GUILayout.MinWidth(40f));
                NGUIEditorTools.SetLabelWidth(80f);
            }
            EditorGUI.EndDisabledGroup();
            GUILayout.EndHorizontal();
