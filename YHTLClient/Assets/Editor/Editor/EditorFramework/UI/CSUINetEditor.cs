...
public class CSUINetEditor : Editor
{
    ...
    private static string tablePath
    {
        get
        {
            ...
            {
                FNDebug.Log("Data/Branch/CurrentUseData/table");
            ...
            {
                FNDebug.Log("Data/Trunk/CurrentUseData/table");
    ...
    static void ReadSelectXml()
    {
        ...
        FNDebug.Log(string.Format("代码生成成功!{0}", behaviourCodeFile));
    ...
    private static XmlNodeList LoadXML(string excelPath)
    {
        if (File.Exists(excelPath))
        {
            ...
            UnityWebRequest www = UnityWebRequest.Get("file:// " + excelPath);
            while (true)
            {
                if (www.isDone)
                {
