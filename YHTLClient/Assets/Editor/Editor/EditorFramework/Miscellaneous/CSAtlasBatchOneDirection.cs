...
{
    public class CSAtlasBatchOneDirection : SelectionBase
    {
        /// <summary>
        ...
        private static int mmDealIndex = -1;
        public static int mDealIndex
        {
            get { return mmDealIndex; }
            set
            {
                mmDealIndex = value;
                // if (mmDealIndex == 0)
        ...
        private static List<MotionSelection> motionList = new List<MotionSelection>();
        static Dictionary<int, bool> directionDic = new Dictionary<int, bool>();
        static EditorWindow win;
        [MenuItem("Tools/Miscellaneous/3002_打Atlas:一个方向一个图集", false, 3003)]
        public static void CSAtlasBatchOneDirectionProc()
        {
            win = GetWindow(typeof(CSAtlasBatchOneDirection));
            win.title = "一个方向一个图集";
        ...
        static void DrawAllMotionSelection()
        {
        ...
        static bool IsSection(string motionName)
        {
            for (int i = 0; i < motionList.Count; i++)
            {
                MotionSelection data = motionList[i];
                if (motionName == data.type.ToString()) return data.isSelect;
            }
            return false;
        }
...
#region GetData
        ...
        static void AddData(FileInfo file)
        {
            ...
            if (!int.TryParse(dirs[1], out data.direction)) return;
            for (int i = 0; i < files.Length; i++)
            {
                FileInfo f = files[i];
                string[] sps = f.Name.Split('_');
                if (f.Extension == ".png" && sps.Length == 3 && dirs[1] == sps[1])
                {
                    if (!IsSection(sps[0])) return;
            ...
            FNDebug.Log(path);
            data.atlasName = data.idInfo.Name + "_" + data.animInfo.Name+"_"+data.direction;
        ...
        static void GetDeepTexFileInfo(DirectoryInfo dir,List<FileInfo> list)
        {
            ...
            if (dirs.Length == 0)
            {
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    {
                        int direction = 0;
                        if(int.TryParse(sps[1],out direction))
                        {
